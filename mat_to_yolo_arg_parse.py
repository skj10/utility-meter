...
if __name__ == "__main__":
    ...
    parser.add_argument('--folder', type=str, default='train', help='number of images')
    
    opt = parser.parse_args()    
    number = opt.number
    folder = opt.folder + '/'
    
    with h5py.File(folder + 'digitStruct.mat') as hdf5_data:
        for i in range(number):
            img_name = get_name(i, hdf5_data)
            #print(img_name)
            im = cv2.imread(folder+img_name)
            h, w, c = im.shape
            arr = get_bbox(i, hdf5_data)
            

            fp = open(folder+img_name.replace('.png','.txt'), 'w')
            arr_l = len(arr['label'])
